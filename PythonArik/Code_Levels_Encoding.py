...
import os
mingw_path = 'C:\\Users\\Arik\\MinGW\\x86_64-5.3.0-release-posix-seh-rt_v4-rev0\\mingw64\\bin'
...
#For Kerwin: you run the original data
#with Initial = True, the rest = False.
# In the function  read_into_panda_from_csv you need to set the names of fields
#You get out a log with next event, total time, remaining time etc.
#Then, you run the Order = True, with the new log, to make the log ordered by time.
#Then, you run the qlength = True, to create Level 1 and Level 2.
#Then you run Multiclass = True to create level 3.

#To do the Machine Learning, you need to run ML = True with the latest log
#Important: In function prep_data you need to select your case attributes (here it is only resource)
#Then you need to toggle the different levels.
#
main(ML=True,qlength=False, Initial=False, Order=False, Multiclass = False)
